      ...
      <div style={{ width:"100%", maxWidth:420, position:"relative", zIndex:1 }}>
        ...
        <div style={{ background:"#ffffff", border:"1px solid #e8dfc8", borderRadius:20, padding:32, boxShadow:"0 8px 40px #00000040" }}>
          ...
          <button onClick={handleSubmit} disabled={loading} style={{
            ...
            color: "#12101f", fontWeight:800, fontSize:14, cursor: loading ? "not-allowed" : "pointer",
            fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center", gap:8,
            boxShadow: loading ? "none" : "0 4px 16px #c9a84c44",
          }}>
...
function exportCoachingPDF(coachingList, coachingLogs) {
  ...
  autoTable(doc, {
    ...
    headStyles: { fillColor: [18, 16, 31], textColor: [201, 168, 76], fontStyle: "bold" },
    ...
    didParseCell: (data) => {
      if (data.section==="body" && data.column.index===0) {
        ...
        if (v==="Critical") data.cell.styles.textColor = [192,57,43];
        if (v==="High") data.cell.styles.textColor = [201,168,76];
        if (v==="Medium") data.cell.styles.textColor = [138,111,40];
      }
    },
  });
...
function MetricCard({ label, value, sub, icon:Icon, color="gold", onClick, alert }) {
  const colors = {
    gold:   { icon:"#c9a84c", bg:"#fdf3d8", border:"#e8c96b" },
    amber:  { icon:"#8a6f28", bg:"#f5ecd4", border:"#c9a84c" },
    warm:   { icon:"#a0845a", bg:"#fdf8f0", border:"#e8dfc8" },
    red:    { icon:"#c0392b", bg:"#fef2f0", border:"#f5a8a8" },
    green:  { icon:"#2e7d32", bg:"#f0faf0", border:"#a5d6a7" },
    orange: { icon:"#c96030", bg:"#fff7ed", border:"#fdba74" },
  };
  ...
  return (
    <div
      ...
      style={{
        background:"#ffffff",
        border:`1px solid ${alert ? "#f5a8a8" : "#e8dfc8"}`,
        borderRadius:14, padding:20,
        cursor:onClick?"pointer":"default",
        transition:"all 0.2s",
        boxShadow: alert ? "0 2px 12px #c0392b11" : "0 1px 4px #c9a84c08",
      }}
      onMouseEnter={e => { if(onClick) e.currentTarget.style.borderColor="#c9a84c"; }}
      onMouseLeave={e => { if(onClick) e.currentTarget.style.borderColor=alert?"#f5a8a8":"#e8dfc8"; }}
    >
...
function FilterSelect({ label, value, onChange, options }) {
  return (
    <select value={value} onChange={e => onChange(e.target.value)}
      style={{ fontSize:13, border:"1px solid #e8dfc8", borderRadius:8, padding:"6px 12px", background:"#ffffff", color:"#1a1510", outline:"none", cursor:"pointer" }}>
...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { id:"dataentry",  label:"Weekly Entry",        icon:ClipboardList },
  ...
  return (
    <div style={{ width:240, minHeight:"100vh", background:"#1b1832", display:"flex", flexDirection:"column", flexShrink:0, borderRight:"1px solid #2e2814" }}>
      ...
      <div style={{ padding:"12px 14px", borderTop:"1px solid #2e2814" }}>
        ...
        <button onClick={onSignOut} style={{ width:"100%", display:"flex", alignItems:"center", gap:8, padding:"6px 10px", borderRadius:8, border:"none", background:"transparent", color:"#c0392b", fontSize:11, cursor:"pointer", fontFamily:"inherit" }}
    ...
    <div style={{ background:"#ffffff", borderBottom:"1px solid #e8dfc8", padding:"12px 28px", display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0, boxShadow:"0 1px 8px #c9a84c10" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:16 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <div style={{ width:32, height:32, borderRadius:"50%", background:"linear-gradient(135deg,#c9a84c,#e8c96b)", display:"flex", alignItems:"center", justifyContent:"center", color:"#12101f", fontSize:12, fontWeight:900 }}>
            {(user?.email?.[0] || "U").toUpperCase()}
          </div>
      ...
      <div className="grid grid-cols-3 gap-5">
        <div style={card}>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16 }}>
            ...
            <h3 style={{ fontWeight:700, color:"#1a1510", fontSize:13, margin:0 }}>Top 5 Performers</h3>
          </div>
          {agg.slice(0,5).map((c,i) => (
            <div key={c.csr_name} onClick={() => onSelectCSR(c)} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 10px", borderRadius:8, cursor:"pointer", marginBottom:4, transition:"background 0.15s" }}
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <p style={{ fontSize:13, fontWeight:600, color:"#1a1510", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{c.csr_name}</p>
                <p style={{ fontSize:11, color:"#7a6a50" }}>{c.team}</p>
              </div>
              <div style={{ textAlign:"right", flexShrink:0 }}>
                <p style={{ fontSize:13, fontWeight:800, color:"#c9a84c" }}>{c.total_rate}</p>
        ...
        <div style={card}>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16 }}>
            <TrendingDown size={14} color="#c0392b" />
            <h3 style={{ fontWeight:700, color:"#1a1510", fontSize:13, margin:0 }}>Bottom 5 Performers</h3>
          </div>
          {agg.slice(-5).reverse().map((c,i) => (
            <div key={c.csr_name} onClick={() => onSelectCSR(c)} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 10px", borderRadius:8, cursor:"pointer", marginBottom:4, transition:"background 0.15s" }}
              ...
              <div style={{ textAlign:"right", flexShrink:0 }}>
                <p style={{ fontSize:13, fontWeight:800, color:"#1a1510" }}>{c.total_rate}</p>
                <StatusBadge status={getStatus(c.total_rate)} />
              </div>
            </div>
          ))}
        ...
        <div style={card}>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16 }}>
            ...
            <h3 style={{ fontWeight:700, color:"#1a1510", fontSize:13, margin:0 }}>Coaching Priority</h3>
          </div>
          {coaching.length === 0
            ? <p style={{ fontSize:13, color:"#7a6a50" }}>No CSRs below 3.50. 🎉</p>
            : coaching.map(c => (
              <div key={c.csr_name} onClick={() => onSelectCSR(c)} style={{ padding:"8px 10px", borderRadius:8, border:"1px solid #fdba74", background:"#fff7ed", cursor:"pointer", marginBottom:8, transition:"background 0.15s" }}
                ...
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4 }}>
                  <p style={{ fontSize:13, fontWeight:600, color:"#1a1510", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", paddingRight:8 }}>{c.csr_name}</p>
                  <StatusBadge status={getStatus(c.total_rate)} />
                </div>
          ...
          {monthlyTrend.length > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={monthlyTrend}>
                ...
                <Line type="monotone" dataKey="avg" name="Total Rate" stroke="#c9a84c" strokeWidth={2.5} dot={{ r:4, fill:"#c9a84c" }} />
        ...
        <div style={card}>
          ...
          {kpiHealth.map(k => (
            <div key={k.name} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
              ...
              <div style={{ flex:1, background:"#e8dfc8", borderRadius:99, height:6 }}>
                <div style={{ height:6, borderRadius:99, width:`${Math.min(k.val,100)}%`, background:k.val>=80?"linear-gradient(90deg,#c9a84c,#8a6f28)":k.val>=70?"#e8c96b":"#c0392b", transition:"width 0.5s" }} />
              </div>
  ...
  return (
    <div style={{ padding:28, background:"#fdf8f0", minHeight:"100%" }} className="space-y-5">
      <SectionHeader title="CSR Ranking" sub="Ranked by Total Rate · submitted entries only">
        <button onClick={() => exportRankingExcel(filtered)} style={{ display:"flex", alignItems:"center", gap:6, padding:"6px 14px", fontSize:12, fontWeight:700, border:"1px solid #e8c96b", borderRadius:8, color:"#8a6f28", background:"#fdf3d8", cursor:"pointer" }}>
              ...
              {filtered.length===0
                ...
                : filtered.map((c,i) => (
                  <tr key={c.csr_name+i} style={{ ...tdBase(i), transition:"background 0.1s" }}
                    onMouseEnter={e=>e.currentTarget.style.background="#fdf3d8"} onMouseLeave={e=>e.currentTarget.style.background=tdBase(i).background}>
                    ...
                    <td style={{ padding:"10px 12px" }}>
                      {c.last_updated_by
                        ? <span style={{ fontSize:11, color:"#7a6a50", display:"flex", alignItems:"center", gap:4 }}><User size={10} />{c.last_updated_by}</span>
    ...
    <div style={{ padding:28, background:"#fdf8f0", minHeight:"100%" }} className="space-y-5">
      ...
      <div style={{ ...card, padding:24 }}>
        <div style={{ display:"flex", alignItems:"flex-start", gap:20 }}>
          <div style={{ width:60, height:60, borderRadius:16, background:"linear-gradient(135deg,#c9a84c,#e8c96b)", display:"flex", alignItems:"center", justifyContent:"center", color:"#12101f", fontSize:20, fontWeight:900, flexShrink:0 }}>
            {(csr.csr_name||"").split(" ").map(n=>n[0]).slice(0,2).join("")}
          </div>
          <div style={{ flex:1 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
              <h2 style={{ fontSize:20, fontWeight:900, color:"#1a1510", margin:0 }}>{csr.csr_name}</h2>
          ...
          <div style={{ textAlign:"right", flexShrink:0 }}>
            <p style={{ fontSize:32, fontWeight:900, color:"#c9a84c", margin:0, lineHeight:1 }}>{csr.total_rate}</p>
            <p style={{ fontSize:11, color:"#7a6a50", marginTop:4 }}>Total Rate (Avg)</p>
          </div>
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginTop:20, paddingTop:20, borderTop:"1px solid #e8dfc8" }}>
          {[["KRA Scale",csr.kra_scale],["Behavioral Scale",csr.behavioral_scale],["Records",csrRecords.length]].map(([l,v])=>(
            <div key={l} style={{ textAlign:"center", padding:12, background:"#fdf8f0", borderRadius:10 }}>
              <p style={{ fontSize:11, color:"#7a6a50", margin:0 }}>{l}</p>
              <p style={{ fontSize:22, fontWeight:900, color:"#1a1510", margin:"4px 0 0" }}>{v}</p>
            </div>
          ))}
        </div>
      </div>
        ...
        <div style={card}>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <RadarChart data={kpiData}>
              ...
              <PolarAngleAxis dataKey="subject" tick={{ fontSize:10, fill:"#7a6a50" }} />
              <PolarRadiusAxis domain={[0,100]} tick={{ fontSize:9, fill:"#a89070" }} />
              <Radar name="Score" dataKey="value" stroke="#c9a84c" fill="#c9a84c" fillOpacity={0.15} />
            </RadarChart>
          </ResponsiveContainer>
        </div>
      </div>
            ...
            <div key={log.id||i} style={{ display:"flex", alignItems:"flex-start", gap:12, padding:12, background:"#fff7ed", borderRadius:8, marginBottom:6, fontSize:11 }}>
              <div style={{ flex:1 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap", marginBottom:4 }}>
                  <span style={{ fontWeight:700, color:"#1a1510" }}>{log.kpi_issue}</span>
            ...
            <tbody>
              {csrRecords.map((r,i) => (
                <tr key={i} style={tdBase(i)}>
                  ...
                  {["conversion_score","rmo_score","rts_score","delivery_success_score","upsell_score"].map(k=>(
                    <td key={k} style={{ padding:"8px 12px", fontWeight:700, color:r[k]<80?"#c0392b":"#1a1510" }}>{parseFloat(r[k]||0).toFixed(1)}%</td>
                  ))}
      ...
      <div style={card}>
        ...
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={chartData} barCategoryGap="30%">
            ...
            <Legend wrapperStyle={{ fontSize:11, color:"#7a6a50" }} />
            <Bar dataKey="avg" name="Team Avg" fill="#c9a84c" radius={[4,4,0,0]} />
            <Bar dataKey="target" name="Target" fill="#e8dfc8" radius={[4,4,0,0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      <div style={{ background:"#ffffff", border:"1px solid #e8dfc8", borderRadius:12, overflow:"hidden" }} className="fade-in">
              ...
              return (
                <tr key={c.name} style={tdBase(i)}>
                  ...
                  <td style={{ padding:"10px 14px", width:140 }}>
                    <div style={{ background:"#e8dfc8", borderRadius:99, height:6 }}>
...
function CoachingTracker({ data, user }) {
  ...
  const pColor = {
    ...
    Medium:  { bg:"#fdf3d8",color:"#7a5c10",border:"#e8c96b" }
  };
  return (
    <div style={{ padding:28, background:"#fdf8f0", minHeight:"100%" }} className="space-y-5">
      <SectionHeader title="Coaching Tracker" sub="Auto-generated from KPI data · changes saved to database">
        <button onClick={()=>exportCoachingPDF(coachingList,initialLogs)} style={{ display:"flex", alignItems:"center", gap:6, padding:"6px 14px", fontSize:12, fontWeight:700, border:"1px solid #f5a8a8", borderRadius:8, color:"#9b2020", background:"#fef2f0", cursor:"pointer" }}>
                      ...
                      <tr key={`${csr.csr_name}-${ii}`} style={tdBase(idx)}>
                        ...
                        {ii===0&&<td rowSpan={issues.length} style={{ padding:"10px 12px", verticalAlign:"top" }}>
                          <input placeholder="Add notes…" value={log.result_notes||""} onChange={e=>updateLog(csr.csr_name,"result_notes",e.target.value)}
                            style={{ fontSize:11, border:"1px solid #e8dfc8", borderRadius:6, padding:"4px 8px", background:"#fdf8f0", color:"#1a1510", outline:"none", width:120 }} />
                        </td>}
                        {ii===0&&<td rowSpan={issues.length} style={{ padding:"10px 12px", verticalAlign:"top", color:"#a89070" }}>
                          {dbLog?(<div><p style={{ fontWeight:700, color:"#7a6a50", margin:0 }}>{dbLog.updated_by||"—"}</p><p style={{ margin:0, fontSize:10 }}>{dbLog.updated_at?new Date(dbLog.updated_at).toLocaleDateString("en-PH",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}):""}</p></div>):<span>—</span>}
                        </td>}
                        {ii===0&&<td rowSpan={issues.length} style={{ padding:"10px 12px", verticalAlign:"top" }}>
                          <button onClick={()=>saveLog(csr)} disabled={saving[csr.csr_name]} style={{ display:"flex", alignItems:"center", gap:4, padding:"5px 12px", borderRadius:8, border:"none", fontSize:11, fontWeight:700, cursor:"pointer", background:saved[csr.csr_name]?"#fdf3d8":"linear-gradient(135deg,#c9a84c,#8a6f28)", color:saved[csr.csr_name]?"#8a6f28":"#12101f", opacity:saving[csr.csr_name]?0.5:1 }}>
      ...
      <div className="grid grid-cols-4 gap-4">
        {quarters.length===0
          ...
          : quarters.map(q=>{
              ...
              return (
                <div key={q} style={{ background:"#ffffff", border:"1px solid #e8c96b", borderRadius:14, padding:20, boxShadow:"0 2px 12px #c9a84c11" }}>
                  <p style={{ fontSize:10, color:"#c9a84c", fontWeight:800, textTransform:"uppercase", letterSpacing:"0.1em", margin:0 }}>{q} {qData[0]?.year||""}</p>
                  <p style={{ fontSize:18, fontWeight:900, color:"#1a1510", marginTop:4 }}>{QUARTERS[q]?.join(" · ")||q}</p>
                  <div style={{ marginTop:12, paddingTop:12, borderTop:"1px solid #e8dfc8" }}>
                    <p style={{ fontSize:11, color:"#7a6a50", margin:0 }}>Team Avg Rate</p>
                    <p style={{ fontSize:26, fontWeight:900, color:"#c9a84c", margin:0 }}>{avg(qData,"total_rate").toFixed(2)}</p>
                    <p style={{ fontSize:11, color:"#7a6a50", marginTop:4 }}>{qData.length} CSRs</p>
                  </div>
                </div>
              );
            })}
            ...
            <tbody>
              {agg.map((c,i)=>(
                <tr key={c.csr_name} style={tdBase(i)}>
                  <td style={{ padding:"10px 14px", fontWeight:700, color:"#1a1510" }}>{c.csr_name}</td>
                  <td style={{ padding:"10px 14px", color:"#7a6a50", fontSize:11 }}>{c.team}</td>
                  <td style={{ padding:"10px 14px", color:"#7a6a50" }}>{c.quarter||"—"}</td>
                  <td style={{ padding:"10px 14px", color:"#7a6a50" }}>{c.month||"—"}</td>
                  <td style={{ padding:"10px 14px", color:"#7a6a50" }}>{c.week||"—"}</td>
                  <td style={{ padding:"10px 14px", fontWeight:900, color:"#c9a84c" }}>{c.total_rate}</td>
                  <td style={{ padding:"10px 14px", color:"#1a1510" }}>{c.kra_scale}</td>
                  <td style={{ padding:"10px 14px", color:"#1a1510" }}>{c.behavioral_scale}</td>
...
function RoadmapCard() {
  ...
  return (
    <div style={{ padding:28, background:"#fdf8f0", minHeight:"100%" }} className="space-y-6">
      <div>
        <h2 style={{ fontSize:20, fontWeight:900, color:"#1a1510", margin:0 }}>Roadmap</h2>
        <p style={{ fontSize:13, color:"#7a6a50", marginTop:4 }}>Version 2.1 — What's been done & what's next</p>
      </div>
      <div className="grid grid-cols-2 gap-4">
        {features.map(({ icon:Icon, label, desc, done }) => (
          <div key={label} style={{ background:"#ffffff", border:`1px solid ${done?"#e8c96b":"#e8dfc8"}`, borderRadius:14, padding:16, display:"flex", alignItems:"flex-start", gap:12 }} className="fade-in">
            <div style={{ width:38, height:38, borderRadius:10, background:done?"#fdf3d8":"#fdf8f0", border:`1px solid ${done?"#e8c96b":"#e8dfc8"}`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
              <Icon size={17} color={done?"#c9a84c":"#a89070"} />
            </div>
            <div>
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <p style={{ fontWeight:700, color:done?"#1a1510":"#7a6a50", fontSize:13, margin:0 }}>{label}</p>
                {done && <span style={{ fontSize:10, fontWeight:800, padding:"1px 7px", borderRadius:99, background:"#fdf3d8", color:"#8a6f28", border:"1px solid #e8c96b" }}>DONE</span>}
              </div>
