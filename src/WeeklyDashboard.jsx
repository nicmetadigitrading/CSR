...
function getStars(score) {
  const full = Math.floor((score / 5) * 5);
  return { full, empty: 5 - full };
}

function autoInsight(score, kraScore, biScore, rmoScore, escScore) {
...
function autoStrengths(record) {
  const kpis = [
    { name:"Conversion/ROAS",      val: record.conversion_kpi_score * 100 },
    { name:"RMO Follow-ups",       val: record.rmo_kpi_score * 100 },
    { name:"RTS Compliance",       val: record.rts_kpi_score * 100 },
    { name:"Delivery Success",     val: record.delivery_success_kpi_score * 100 },
    { name:"Upsell Rate",          val: record.upsell_kpi_score * 100 },
    { name:"ESC",                  val: record.esc_kpi_score * 100 },
  ].filter(k => k.val !== null && !isNaN(k.val)).sort((a, b) => b.val - a.val);

  const top = kpis.slice(0, 3).map(k => k.name);
  return top.length > 0 ? top.join(", ") : "Consistent effort shown across all areas.";
}
...
function autoActionPlan(record) {
  const finalScore = record.final_score || 0;
  if (finalScore >= 4.5) return "Continue your excellent practices and help mentor lower-performing team members.";
  if (finalScore >= 4.0) return "Focus on your weakest KPI this week. Set a daily target and track your progress consistently.";
  if (finalScore >= 3.5) return "Schedule a 1-on-1 with your TL. Review your call recordings and identify specific improvement areas.";
...
function autoFocus(record) {
  const kpis = [
    { name:"RMO Follow-ups",       val: (record.rmo_kpi_score || 0) * 100 },
    { name:"Conversion/ROAS",      val: (record.conversion_kpi_score || 0) * 100 },
    { name:"RTS Compliance",       val: (record.rts_kpi_score || 0) * 100 },
    { name:"Upsell Rate",          val: (record.upsell_kpi_score || 0) * 100 },
    { name:"Delivery Success",     val: (record.delivery_success_kpi_score || 0) * 100 },
    { name:"ESC",                  val: (record.esc_kpi_score || 0) * 100 },
  ].sort((a, b) => a.val - b.val);
  const lowest = kpis[0];
  return `Improve ${lowest.name} rate and maintain all other KPI targets consistently.`;
}

function autoGoal(record) {
  const finalScore = record.final_score || 0;
  if (finalScore >= 4.5) return "Maintain Outstanding performance and be a model for the team.";
  if (finalScore >= 4.0) return "Achieve higher scores consistently and break into the Outstanding tier.";
  if (finalScore >= 3.5) return "Hit all KPI targets and move up to the Good performance tier.";
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
      ...
      <div style={{ textAlign: "center" }}>
        <div style={{ fontSize: 22, letterSpacing: 2 }}>
          {"★".repeat(stars.full)}<span style={{ color: "#334155" }}>{"☆".repeat(stars.empty)}</span>
        </div>
...
function BehavioralCard({ label, value, icon }) {
  const pct = value !== null ? (value / 5) * 100 : null;
  ...
  return (
    <div style={{ background: "#0d1729", border: "1px solid #1e293b", borderRadius: 12, padding: "14px 12px", textAlign: "center" }}>
      ...
      <div style={{ fontSize: 18, fontWeight: 900, color: pct !== null ? color : "#334155" }}>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080f1f", fontFamily: "'Inter','DM Sans',system-ui,sans-serif", color: "#e2e8f0", padding: "0 0 80px" }}>
      ...
      <div style={{ background: "#0d1729", borderBottom: "1px solid #1e293b", padding: "16px 32px", display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <select value={selectedCSR} onChange={e => setSelectedCSR(e.target.value)} style={{ ...inputStyle, minWidth: 220 }}>
          <option value="">Select CSR…</option>
          {CSR_NAMES.map(n => <option key={n} value={n}>{n}</option>)}
        </select>
        <select value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)} style={{ ...inputStyle, minWidth: 140 }}>
          <option value="">Select month…</option>
          {MONTHS.map(m => <option key={m} value={m}>{m}</option>)}
        </select>
        <select value={selectedWeek} onChange={e => setSelectedWeek(e.target.value)} style={{ ...inputStyle, minWidth: 120 }}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px" }}>
          ...
          <div style={{
            ...
            <div style={{ flex: 1, minWidth: 200 }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 900, color: "#fff", lineHeight: 1.1, marginBottom: 8 }}>
                {record.csr_name}
              </div>
              ...
              <div style={{ background: "#0f172a88", border: "1px solid #334155", borderRadius: 10, padding: "12px 16px", marginTop: 8, maxWidth: 340 }}>
                ...
                <p style={{ fontSize: 12, color: "#cbd5e1", lineHeight: 1.6, margin: 0 }}>
                  {autoInsight(record.final_score, record.kra_total, record.bi_score, record.rmo_kpi_score, record.esc_kpi_score)}
                </p>
              </div>
            </div>

            {/* Score + KRA/Behavioral */}
            <div style={{ display: "flex", gap: 20, alignItems: "flex-start", flexWrap: "wrap" }}>
              <div style={{ textAlign: "center" }}>
                ...
                <ScoreRing score={record.final_score || 0} />
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 12, minWidth: 160 }}>
                <div style={{ background: "#052e16", border: "1px solid #166534", borderRadius: 12, padding: "14px 20px" }}>
                  ...
                  <div style={{ fontSize: 24, fontWeight: 900, color: "#22c55e" }}>{record.kra_total ? (record.kra_total * 100).toFixed(2) + "%" : "—"}</div>
                  <div style={{ fontSize: 13, color: "#16a34a", marginTop: 2 }}>KRA Scale: <strong>{record.kra_total ? (record.kra_total * 5).toFixed(2) : "—"}</strong></div>
                </div>
                <div style={{ background: "#052e16", border: "1px solid #166534", borderRadius: 12, padding: "14px 20px" }}>
                  ...
                  <div style={{ fontSize: 24, fontWeight: 900, color: "#22c55e" }}>{record.bi_score ? (record.bi_score * 100).toFixed(2) + "%" : "—"}</div>
                  <div style={{ fontSize: 13, color: "#16a34a", marginTop: 2 }}>Behavioral Scale: <strong>{record.bi_score ? (record.bi_score * 5).toFixed(2) : "—"}</strong></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
            ...
            <div style={{ background: "#0d1729", border: "1px solid #1e293b", borderRadius: 12, padding: 20 }}>
              ...
              <KpiRow label="Follow-Ups / RMO" value={record.rmo_kpi_score !== null ? record.rmo_kpi_score * 100 : null} />
              <KpiRow label="Verified Calls" value={record.g_4_1_4 !== null ? record.g_4_1_4 * 20 : null} />
              ...
              <KpiRow label="ROAS Performance"       value={record.conversion_kpi_score !== null ? record.conversion_kpi_score * 100 : null} />
              <KpiRow label="RTS Compliance"         value={record.rts_kpi_score !== null ? record.rts_kpi_score * 100 : null} />
              <KpiRow label="Sales Encoding Accuracy" value={record.g_6_1_1 !== null ? record.g_6_1_1 * 20 : null} />
              <KpiRow label="Upsell Rate"            value={record.upsell_kpi_score !== null ? record.upsell_kpi_score * 100 : null} />
            </div>
            ...
            <div style={{ background: "#0d1729", border: "1px solid #1e293b", borderRadius: 12, padding: 20 }}>
              ...
              {[
                { icon:"🚚", label:"RTS %",               val: record.rts_pct !== null ? (record.rts_pct * 100).toFixed(2) + "%" : "—", warn: record.rts_pct > 0.15 },
                { icon:"✅", label:"Delivery Success",    val: record.delivery_success_rate !== null ? (record.delivery_success_rate * 100).toFixed(2) + "%" : "—" },
                { icon:"📞", label:"Weekly RMO Rate",     val: record.weekly_rmo_rate !== null ? (record.weekly_rmo_rate > 1 ? record.weekly_rmo_rate.toFixed(2) : (record.weekly_rmo_rate * 100).toFixed(2)) + "%" : "—", warn: record.weekly_rmo_rate < 0.55 },
                { icon:"⭐", label:"ESC Points",          val: record.esc_points !== null ? record.esc_points : "—", warn: record.esc_points < 9 },
                { icon:"📈", label:"Conversion (ROAS)",   val: record.conversion_roas !== null ? record.conversion_roas : "—" },
                { icon:"🏷", label:"Upsell Rate",         val: record.upsell_rate !== null ? (record.upsell_rate > 1 ? record.upsell_rate.toFixed(2) : (record.upsell_rate * 100).toFixed(2)) + "%" : "—" },
                ...
                <div key={label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #1e293b11" }}>
            ...
            <div style={{ background: "#0d1729", border: "1px solid #1e293b", borderRadius: 12, padding: 20 }}>
              ...
              {[
                { label:"Business Process", val: record.kra_bp, icon:"⚙️",   color:"#6366f1" },
                { label:"Customer",         val: record.kra_customer, icon:"👥", color:"#0ea5e9" },
                { label:"People Development", val: record.kra_people, icon:"👤", color:"#10b981" },
                { label:"Financial",        val: record.kra_financial, icon:"💰", color:"#f59e0b" },
              ...
              <div style={{ marginTop: 16, padding: "10px 14px", background: "#080f1f", borderRadius: 8, borderTop: "2px solid #6366f1" }}>
                ...
                <div style={{ fontSize: 22, fontWeight: 900, color: "#22c55e", marginTop: 2 }}>
                  {record.kra_total ? (record.kra_total * 100).toFixed(2) + "%" : "—"}
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ background: "#0d1729", border: "1px solid #1e293b", borderRadius: 12, padding: 20, marginBottom: 16 }}>
            ...
            <div style={{ padding: "10px 16px", background: "#080f1f", borderRadius: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <span style={{ fontSize: 20, fontWeight: 900, color: record.bi_score >= 0.8 ? "#22c55e" : record.bi_score >= 0.6 ? "#f59e0b" : "#ef4444" }}>
                {record.bi_score ? (record.bi_score * 100).toFixed(2) + "%" : "—"}
              </span>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
            <div style={{ background: "#0d1729", border: "1px solid #1e293b", borderRadius: 12, padding: 20 }}>
              {(() => {
                const status = getStatus(record.final_score || 0);
                return (
                  <>
                    ...
                    <p style={{ fontSize: 12, color: "#94a3b8", lineHeight: 1.7, marginBottom: 14 }}>
                      {autoInsight(record.final_score, record.kra_total, record.bi_score)}
                    </p>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      <div style={{ display: "flex", gap: 8 }}>
                        <span style={{ fontSize: 14 }}>⭐</span>
                        <div><span style={{ fontSize: 11, fontWeight: 700, color: "#22c55e" }}>Strengths: </span><span style={{ fontSize: 11, color: "#94a3b8" }}>{autoStrengths(record)}</span></div>
                      </div>
                      <div style={{ display: "flex", gap: 8 }}>
                        <span style={{ fontSize: 14 }}>📈</span>
                        <div><span style={{ fontSize: 11, fontWeight: 700, color: "#f59e0b" }}>Opportunities: </span><span style={{ fontSize: 11, color: "#94a3b8" }}>{autoOpportunities(record)}</span></div>
                      </div>
                      <div style={{ display: "flex", gap: 8 }}>
                        <span style={{ fontSize: 14 }}>🎯</span>
                        <div><span style={{ fontSize: 11, fontWeight: 700, color: "#0ea5e9" }}>Action Plan: </span><span style={{ fontSize: 11, color: "#94a3b8" }}>{autoActionPlan(record)}</span></div>
                      </div>
                    </div>
                  </>
                );
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {[
                ...
                { icon:"🏁", label:"GOAL",       color:"#22c55e", text: autoGoal(record) },
