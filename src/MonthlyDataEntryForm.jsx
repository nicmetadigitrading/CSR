...
export default function MonthlyDataEntryForm({ user }) {
  ...
  const handleSave = async (saveStatus) => {
    ...
    else { setEntryStatus(saveStatus); await refreshDrafts(); showToast("success", saveStatus === "draft" ? `📝 Draft saved for ${resolvedName} — ${selectedMonth}` : `✅ Submitted for ${resolvedName} — ${selectedMonth}`); }
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, fontFamily: "'Inter','DM Sans',system-ui,sans-serif", color: T.text, padding: "0 0 80px" }}>
      ...
      <div style={{ background: "#1b1832", borderBottom: "1px solid #2e2814", padding: "14px 32px", display: "flex", alignItems: "center", gap: 12, position: "sticky", top: 0, zIndex: 50, boxShadow: "0 2px 12px #00000020" }}>
        ...
        <div>
          <div style={{ fontWeight: 800, fontSize: 15, color: "#f5ecd4" }}>Monthly CSR Performance Entry</div>
